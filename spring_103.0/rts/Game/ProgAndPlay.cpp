...
#include "Map/ReadMap.h" //mapDims.mapx & mapDims.mapy
...
void CProgAndPlay::Update(void) {
	...
	bool askHelp = configHandler->IsSet("helpPlease") && configHandler->GetString("helpPlease").compare("enabled") == 0;
	if (askHelp){
		log("CProgAndPlay : user ask help");
		configHandler->SetString("helpPlease", "", true); // reset help notification
	}

	// Check if mission is ended. This depends on engine state (game->IsGameOver()) and/or
	// missions state (victoryState)
	if (!missionEnded) {
		std::string victoryState = configHandler->IsSet("victoryState") ? configHandler->GetString("victoryState") : "";
		if (victoryState.compare("") != 0 || game->IsGameOver()) {
			// Log end messages
			if (traceModuleCorrectlyInitialized) {
		...
		if (tp->compressionDone()){
			...
			if (!testMapMode) {
				...
				if (missionEnded) {
					...
					bool dirExists = FileSystem::CreateDirectory(springDataPath);
					log("Try to create: " + springExpertPath);
					dirExists = FileSystem::CreateDirectory(springExpertPath);
					if (dirExists) {
						std::string path = springExpertPath + missionName;
						log("Try to create: " + path);
						dirExists = FileSystem::CreateDirectory(path);
						if (dirExists) {
	...
	if (configHandler->IsSet("publish") && configHandler->GetString("publish").compare("true") == 0) {
...
int CProgAndPlay::updatePP(void){
	...
	if (!updated) {
		...
		// store map size
		PP_Pos mapSize;
		mapSize.x = mapDims.mapx*SQUARE_SIZE;
		mapSize.y = mapDims.mapy*SQUARE_SIZE;
	...
	resources.resource[0] = teamHandler->Team(gu->myTeam)->res.metal;
	resources.resource[1] = teamHandler->Team(gu->myTeam)->res.energy;
		...
		for (int i = 0 ; i < pendingCommands->size ; i++){
			...
			switch (pc.commandType){
				...
				case 0 : // the command target a position
					...
					pc.command.param[1] = CGround::GetHeightReal(pc.command.param[0], pc.command.param[2]);
					break;
			...
			if (ok){
				if (pc.command.code == -7658){ // This code is a Prog&Play reserved code defined in constantList files
					...
					clientNet->Send(CBaseNetProtocol::Get().SendAICommand(gu->myPlayerNum, 
						skirmishAIHandler.GetCurrentAIID(),  pc.unitId, pc.command.code, -1, 0, stdParam));
/*					// Permet de gérer les commandes plus correctement mais produit des
					// erreurs de synchronisation (à utiliser en SYNCED_MODE)
					std::vector<CUnit*> *tmp = &(gs->Team(gu->myTeam)->units);
					std::vector<CUnit*>::iterator it = tmp->find(pc.unitId);
...
bool CProgAndPlay::allUnitsIdled() {
	std::vector<CUnit*> *tmp = &(teamHandler->Team(gu->myTeam)->units);
	if (tmp->size() == 0)
		return false;
	std::vector<CUnit*>::iterator it = tmp->begin();
	while (it != tmp->end()) {
...
void CProgAndPlay::initTracesFile() {
	...
	bool dirExists = FileSystem::CreateDirectory(springTracesPath);
	if (dirExists) {
...
void CProgAndPlay::sendRequestToServer() {
	...
	try {
		...
		request_stream << "GET " << server_path << "?score=" << (configHandler->IsSet("score") ? configHandler->GetString("score") : "") << "&mission_name=" << missionName << " HTTP/1.0\r\n";
